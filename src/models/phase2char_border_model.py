...
class CNNBackbone(nn.Module):
    def __init__(self):
        ...
        self.conv_1 = nn.Conv2d(
            3, 4, kernel_size=3, stride=1, padding="same", bias=True
        )
        self.conv_2 = nn.Conv2d(
            4, 8, kernel_size=3, stride=1, padding="same", bias=True
        )
        ...
        self.conv_final = nn.Conv2d(
            32, 32, kernel_size=4, padding="valid", bias=False
        )
...
class CNNModulePhase2CharsBorder(pl.LightningModule):
    ...
    def _loss(self, batch: Dict[str, Tensor], char_end_x: Tensor, name: str):
        ...
        loss = (char_end_x - gt_char_end_x) ** 2 + 10.0 * torch.abs(
            char_end_x - gt_char_end_x
        )
    ...
    def inference(self, line_image: Image.Image) -> List[float]:
        ...
        while x_coords[-1] < line_image.width:
            img_crop = line_image.crop(
                (x_coords[-1], 0, x_coords[-1] + height, height)
            )

            input_tensor = Phase2CharBorderDataset.TRANSFORMS(
                img_crop
            ).unsqueeze(0)
