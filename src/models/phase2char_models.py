...
class CNNBackbone(nn.Module):
    def __init__(self):
        ...
        self.conv_1 = nn.Conv2d(
            3, 8, kernel_size=5, stride=2, padding=2, bias=True
        )
        self.conv_2 = nn.Conv2d(
            8, 16, kernel_size=3, stride=1, padding="same", bias=True
        )
        self.conv_3 = nn.Conv2d(
            16, 32, kernel_size=3, stride=1, padding="same", bias=True
        )
        self.conv_4 = nn.Conv2d(
            32, 64, kernel_size=3, stride=1, padding="same", bias=True
        )
        self.conv_5 = nn.Conv2d(
            64, 128, kernel_size=3, stride=1, padding="same", bias=True
        )
        self.conv_final = nn.Conv2d(
            128, 128, kernel_size=2, padding="valid", bias=False
        )
...
class FCNForChars(nn.Module):
    def __init__(
        ...
        tiny_h: int = 16,
        full_h: int = 128,
        num_classes: int = len(ALL_CHARS) + 1,
    ):
        super().__init__()
        ...
        self.fc_2 = nn.Linear(
            (self.in_dim + num_classes) // 2, (self.in_dim + num_classes) // 2
        )
        self.fc_3 = nn.Linear(
            (self.in_dim + num_classes) // 2, self.num_classes
        )
...
class CNNModulePhase2Chars(pl.LightningModule):
    def __init__(
        ...
        self.image_feature_reduce = ImageFeatureReduce(
            full_h=full_h, tiny_h=tiny_h
        )
        self.last_fcn = FCNForChars(
            w=w, tiny_h=tiny_h, full_h=full_h, num_classes=num_classes
        )
    ...
    def forward(
        ...
        concat_image_features = torch.concat(
            [tiny_pre_features, actual_image_feature, tiny_post_features], dim=1
        )
    ...
    def _loss(
        ...
        loss_width = (
            torch.abs(char_width_gt - char_width.flatten()) * width_factor
        )
    ...
    def inference(
        ...
        while x_offset < line_image.width:
            img = line_image.crop(
                (x_offset, 0, x_offset + line_image.height, line_image.height)
            )
            ...
            char_width_relative = self.fcn_for_char_width(
                image_feature[:, :, 0, 0]
            ).item()
            ...
            tiny_feature: Tensor = self.image_feature_reduce(
                image_feature[:, :, 0, 0]
            )
            tiny_feature_list.append(tiny_feature.flatten())

        black_image = Image.new(
            "RGB", (line_image.height, line_image.height), color=(0, 0, 0)
        )
        black_feature = self.cnn_backbone(
            Phase2CharDataset.TRANSFORMS(black_image).unsqueeze(0)
        )
        tiny_black_feature = self.image_feature_reduce(
            black_feature[:, :, 0, 0]
        ).flatten()
        ...
        for idx in range(len(feature_list)):
            ...
            for jj in range(num_not_black_features):
                tiny_pre_features.append(
                    tiny_feature_list[idx - num_not_black_features + jj]
                )
            ...
            all_features = torch.cat(
                tiny_pre_features + [feature_list[idx]] + tiny_post_features,
                dim=0,
            ).unsqueeze(0)
