from pytorch_lightning import Trainer
from src.image_dataset import ImageDataset
from src.image_reader import ImageReader
from src.models import CNNModule2Points, CNNModule6Points
from src.reader import Top2PointsDataset
from torch.utils.data import DataLoader
from pathlib import Path
from pytorch_lightning.callbacks import ModelCheckpoint
import time
import argparse

PROJ_DIR = Path("/home/avandavad/projects/receipt_extractor")


def main(args):
    train_reader = ImageReader(PROJ_DIR / "data" / "train")
    val_reader = ImageReader(PROJ_DIR / "data" / "val")

    train_dataset = Top2PointsDataset(train_reader, augment=True)
    val_dataset = Top2PointsDataset(val_reader, augment=False)

    train_dataloader = DataLoader(train_dataset, batch_size=16, num_workers=4)
    val_dataloader = DataLoader(val_dataset, batch_size=16, num_workers=4)

    if args.from_scratch:
        model = CNNModule2Points()
    else:
        version_num = 1
        ckpt_name = "resnet-epoch=56-val_loss=0.00995"
        ckpt_path = (
            PROJ_DIR
            / "model_checkpoints"
            / "CNNModule2Points"
            / "lightning_logs"
            / f"version_{version_num}"
            / "checkpoints"
            / f"{ckpt_name}.ckpt"
        )
        model = CNNModule2Points().load_from_checkpoint(ckpt_path)

    checkpoint_callback = ModelCheckpoint(
        monitor="val_loss",
        filename="resnet-{epoch:02d}-{val_loss:.5f}",
        save_top_k=3,
        mode="min",
    )

    trainer = Trainer(
        default_root_dir=PROJ_DIR / "model_checkpoints" / "CNNModule2Points",
        accelerator="gpu",
        max_epochs=200,
        callbacks=[checkpoint_callback],
        log_every_n_steps=3,
    )
    trainer.validate(model, val_dataloader)

    print("starting training...")
    time.sleep(5)

    trainer.fit(
        model,
        train_dataloader,
        val_dataloader,
    )

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--from_scratch", action="store_true")

    args = parser.parse_args()
    main(args)
